...
<div class="uk-article edit item-page<?php echo $this->pageclass_sfx; ?>">
    ...
    <form action="<?php echo Route::_('index.php?option=com_content&a_id=' . ( int )$this->item->id); ?>" method="post" name="adminForm" id="adminForm" class="form-validate form-vertical">
        ...
        <div class="uk-flex">
            <button type="button" class="uk-button uk-button-primary uk-margin-small-right" onclick="Joomla.submitbutton( 'article.save' )"><?php echo ($jsIcons !== 'none' ? '<span class="uk-margin-small-right" data-uk-icon="icon:ok"></span>' : ''), Text::_('JSAVE') ?></button>
            <button type="button" class="uk-button uk-button-default uk-margin-small-right" onclick="Joomla.submitbutton( 'article.cancel' )"><?php echo ($jsIcons !== 'none' ? '<span class="uk-margin-small-right" data-uk-icon="icon:cancel"></span>' : ''), Text::_('JCANCEL') ?></button>
